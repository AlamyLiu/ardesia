...
static void check_composite_manager(
    )
{
...
static CommandLine *parse_options(
    gint argc,
    char *argv[])
{
    ...
    while (1) {
        ...
        gint option_index = 0;
        c = getopt_long(argc,
                        argv, "hdvVg:f:l:t:", long_options, &option_index);
...
static void create_workspace_shortcut(
    gchar * workspace_dir)
{
...
#else
    xdg_create_link(workspace_dir, desktop_entry_filename,
                    "folder-documents");
#endif
...
static gchar *configure_workspace(
    gchar * project_name)
{
    ...
    workspace_dir =
        g_build_filename(documents_dir, PACKAGE_NAME, (gchar *) 0);
...
static gchar *create_default_project_dir(
    gchar * workspace_dir,
    gchar * project_name)
{
    ...
    char *argv[])
{
    ...
    if (commandline->iwb_filename) {
        ...
        if (g_path_is_absolute(commandline->iwb_filename)) {
            ...
            iwb_filename =
                g_build_filename(dir, commandline->iwb_filename, (gchar *) 0);
            g_free(dir);
        }
