...
static gboolean start_save_pdf_dialog(
    GtkWindow * parent,
    GdkPixbuf * pixbuf)
{
    gboolean ret = TRUE;
    ...
    preview_pixbuf =
        gdk_pixbuf_scale_simple(pixbuf, preview_width, preview_height,
                                GDK_INTERP_BILINEAR);
...
static gboolean init_pdf_saver(
    GtkWindow * parent,
    GdkPixbuf * pixbuf)
{
    gboolean ret = FALSE;
    ...
    GtkWindow * parent)
{
    ...
    gchar *screenshoot_name =
        g_strdup_printf("%s_screenshoot.png", default_filename);
    gchar *filename =
        g_build_filename(tmp_dir, screenshoot_name, (gchar *) 0);
    GError *err = NULL;
    ...
    pdf_data->input_filelist =
        g_slist_prepend(pdf_data->input_filelist, filename);

    wait_for_pdf_save_pending_thread();

    /* Start save thread. */
    if ((pdf_data->thread =
         g_thread_create((GThreadFunc) pdf_save, (void *)NULL, TRUE,
                         &err)) == NULL) {
    ...
    )
{
    if (pdf_data) {
        ...
        while (pdf_data->input_filelist) {
            gchar *filename =
                (gchar *) g_slist_nth_data(pdf_data->input_filelist,
                                           0);
            if (filename) {
                g_remove(filename);
                pdf_data->input_filelist =
                    g_slist_remove(pdf_data->input_filelist, filename);
                g_free(filename);
                filename = NULL;
            }
        }
