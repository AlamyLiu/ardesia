...
static void add_background_image_reference(
...
static void add_background_color_reference(
    xmlXPathContextPtr context,
    xmlChar * ref)
{
    xmlChar *xpath =
        (xmlChar *) g_strdup_printf("/iwb/svg:svg/svg:rect[@id='%s']",
                                    (gchar *) ref);
    ...
    gchar *blue = g_substr((const gchar *)fill, comma_index + 1,
    ...
    gchar *remain_string =
        g_substr((const gchar *)fill, open_bracket_index + 1,
                 comma_index - 1);
    ...
    gchar *green = g_substr((const gchar *)remain_string, comma_index + 1,
                            strlen(remain_string) - 1);
...
void load_background_by_reference(
    gchar * project_tmp_dir,
    xmlXPathContextPtr context,
    xmlChar * ref)
{
    xmlChar *xpath =
        (xmlChar *) g_strdup_printf("/iwb/svg:svg/svg:image[@id='%s']",
                                    (gchar *) ref);
    xmlXPathObjectPtr result = xmlXPathEvalExpression(xpath, context);
...
/* Follow the ref and load the associated save-point. */
GSList *load_savepoint_by_reference(
    ...
    xmlChar * ref)
{
    ...
    savepoint->filename =
        g_build_filename(project_tmp_dir, href, (gchar *) 0);
    ...
    gchar * dest_dir)
{
    ...
    for (j = 0; j < gsf_infile_num_children(infile); j++) {
        ...
        if (is_dir) {
            gchar *dir_path =
                g_build_filename(dest_dir, filename, (gchar *) 0);
            ...
            gchar *file_path =
                g_build_filename(dest_dir, filename, (gchar *) 0);
            GsfOutput *output =
                GSF_OUTPUT(gsf_output_stdio_new(file_path, &err));
    ...
    gchar * project_tmp_dir)
{
    ...
    if (infile == NULL) {
        g_warning("Error decompressing iwb file %s: %s", iwbfile,
                  err->message);
        g_error_free(err);
        return;
    }
...
static xmlXPathContextPtr register_namespaces(
    xmlXPathContextPtr context)
{
    xmlXPathRegisterNs(context, (xmlChar *) "iwb",
                       (xmlChar *) "http://www.becta.org.uk/iwb");
    xmlXPathRegisterNs(context, (xmlChar *) "xlink",
...
static GSList *load_savepoints_by_iwb(
    GSList * savepoint_list,
    gchar * project_tmp_dir,
    xmlXPathContextPtr context)
{
    ...
    xmlXPathObjectPtr result =
        xmlXPathEvalExpression(xpath_get_element, context);
    gint i = 0;
    ...
    for (i = 0; i < result->nodesetval->nodeNr; i++) {
        ...
        if (background) {
            ...
            savepoint_list =
                load_savepoint_by_reference(savepoint_list,
                                            project_tmp_dir, context, ref);
        }
        xmlFree(ref);
    }
    ...
    gchar * iwbfile)
{
    ...
    gchar *content_filepath =
        g_build_filename(project_tmp_dir, content_filename, (gchar *) 0);
    ...
    savepoint_list =
        load_savepoints_by_iwb(savepoint_list, project_tmp_dir, context);
