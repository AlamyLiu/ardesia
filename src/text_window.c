    ...
    GtkWindow * parent)
{
    ...
    if (!text_data->text_window_gtk_builder) {
        ...
        gtk_builder_add_from_file(text_data->text_window_gtk_builder,
                                  TEXT_UI_FILE, &error);
    ...
    if (!text_data->window) {
        GObject *text_obj =
            gtk_builder_get_object(text_data->text_window_gtk_builder,
                                   "text_window");
        text_data->window = GTK_WIDGET(text_obj);

        gtk_window_set_opacity(GTK_WINDOW(text_data->window), 1);
        gtk_widget_set_size_request(GTK_WIDGET(text_data->window),
...
/* Blink cursor. */
static gboolean blink_cursor(
    gpointer data)
{
    if ((text_data->window) && (text_data->pos)) {
        cairo_t *cr =
            gdk_cairo_create(gtk_widget_get_window(text_data->window));
    ...
    )
{
    ...
    if (char_info) {
        ...
        text_data->letterlist =
            g_slist_remove(text_data->letterlist, char_info);
    }
...
/* Set the text cursor. */
static gboolean set_text_cursor(
    GtkWidget * window)
{
    ...
    if (text_pointer_cr) {
        ...
        cairo_line_to(text_pointer_cr, 2 * width / 3 + 1,
        ...
        cairo_line_to(text_pointer_cr, width / 3 - 1,
                      height - decoration_height);
    ...
    cursor =
        gdk_cursor_new_from_pixbuf(gdk_display_get_default(), pixbuf,
...
G_MODULE_EXPORT gboolean key_snooper(
    GtkWidget * widget,
    GdkEventKey * event,
    gpointer user_data)
{
    ...
    gboolean closed_to_bar =
        inside_bar_window(text_data->pos->x + text_data->extents.x_advance,
    ...
    else if ((text_data->pos->x + text_data->extents.x_advance >=
              gdk_screen_width()) || (closed_to_bar)
    ...
    /* Is the character printable? */
        ...
        text_data->letterlist =
            g_slist_prepend(text_data->letterlist, char_info);
...
/* On configure event. */
G_MODULE_EXPORT gboolean on_text_window_configure(
    GtkWidget * widget,
...
/* On screen changed. */
G_MODULE_EXPORT void on_text_window_screen_changed(
    GtkWidget * widget,
...
G_MODULE_EXPORT gboolean on_text_window_expose_event(
    GtkWidget * widget,
    cairo_t * cr,
    gpointer data)
{
...
static gboolean is_above_virtual_keyboard(
    gint x,
    gint y)
{
    RECT rect;
...
G_MODULE_EXPORT gboolean on_text_window_button_release(
    GtkWidget * win,
    GdkEventButton * ev,
    gpointer user_data)
{
...
G_MODULE_EXPORT gboolean on_text_window_cursor_motion(
    GtkWidget * win,
    GdkEventMotion * ev,
    gpointer func_data)
{
    ...
    gint tickness)
{
    ...
    gtk_builder_connect_signals(text_data->text_window_gtk_builder,
                                (gpointer) text_data);

    /* Install the key snooper. */
    text_data->snooper_handler_id =
    ...
    setLayeredGdkWindowAttributes(gtk_widget_get_window(text_data->window),
