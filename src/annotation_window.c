...
static AnnotatePaintContext *annotate_paint_context_new(
    ...
    AnnotateDeviceData * devdata)
{
    ...
    GSList *relevantpoint_list =
        build_meaningful_point_list(out_ptr, FALSE, tollerance);
...
{
    guint refcount =
        (guint) cairo_get_reference_count(data->annotation_cairo_context);
 ...
 */
static void annotate_draw_ellipse(
    AnnotateDeviceData * devdata,
    gdouble x,
    gdouble y,
    gdouble width,
...
static void annotate_draw_curve(
    AnnotateDeviceData * devdata,
    GSList * list)
{
    ...
    if (list) {
        ...
        for (i = 0; i < lenght; i = i + 3) {
            AnnotatePoint *first_point =
                (AnnotatePoint *) g_slist_nth_data(list, i);
            if (!first_point) {
                return;
            }
            if (lenght == 1) {
                /* It is a point. */
                annotate_draw_point(devdata, first_point->x,
                                    first_point->y, first_point->pressure);
            } else {
                ...
                if (!second_point) {
                    ...
                    AnnotatePoint *third_point = (AnnotatePoint *)
    ...
    gboolean closed_path)
{
    ...
    GSList *broken_list =
        broken(devdata->coord_list, closed_path, TRUE, tollerance);
    ...
    gboolean closed_path)
{
    ...
    meaningful_point_list =
        build_meaningful_point_list(devdata->coord_list, closed_path,
                                    tollerance);
    ...
    if (g_slist_length(meaningful_point_list) < 4) {
        ...
        annotate_draw_point_list(devdata, meaningful_point_list);
               ...
               (is_similar_to_an_ellipse
                (meaningful_point_list, tollerance))) {
        GSList *rect_list = build_outbounded_rectangle(meaningful_point_list);

        if (rect_list) {
            AnnotatePoint *point1 =
                (AnnotatePoint *) g_slist_nth_data(rect_list, 0);
            AnnotatePoint *point2 =
                (AnnotatePoint *) g_slist_nth_data(rect_list, 1);
            AnnotatePoint *point3 =
                (AnnotatePoint *) g_slist_nth_data(rect_list, 2);
            gdouble p1p2 = get_distance(point1->x, point1->y, point2->x,
                                        point2->y);
            gdouble p2p3 = get_distance(point2->x, point2->y, point3->x,
                                        point3->y);
            ...
            annotate_draw_ellipse(devdata, point1->x, point1->y,
                                  point3->x - point1->x,
                                  point3->y - point1->y, point1->pressure);
    ...
    )
{
    ...
    gtk_builder_add_from_file(data->annotation_window_gtk_builder,
    ...
    widget =
        GTK_WIDGET(gtk_builder_get_object
                   (data->annotation_window_gtk_builder, "annotationWindow"));
    ...
    GtkWidget * parent)
{
    ...
    g_signal_connect(gdk_display_get_device_manager
                     (gdk_display_get_default()), "device-added",
                     G_CALLBACK(on_device_added), data);
    g_signal_connect(gdk_display_get_device_manager
                     (gdk_display_get_default()), "device-removed",
    ...
    setLayeredGdkWindowAttributes(gtk_widget_get_window
                                  (data->annotation_window), RGB(0, 0, 0),
                                  0, LWA_COLORKEY);
#endif
}
    ...
    )
{
    ...
    gchar *ardesia_tmp_dir =
        g_build_filename(tmpdir, PACKAGE_NAME, (gchar *) 0);
    gchar *project_tmp_dir =
        g_build_filename(ardesia_tmp_dir, project_name, (gchar *) 0);
    ...
    data->savepoint_dir =
        g_build_filename(project_tmp_dir, images, (gchar *) 0);
...
static void delete_savepoint(
    AnnotateSavepoint * savepoint)
{
    if (savepoint) {
        ...
        data->savepoint_list =
            g_slist_remove(data->savepoint_list, savepoint);
        g_free(savepoint);
    ...
    )
{
    ...
    while (data->savepoint_list != stop_list) {
        AnnotateSavepoint *savepoint =
            (AnnotateSavepoint *) g_slist_nth_data(data->savepoint_list,
                                                   0);
...
{
    gchar *ardesia_tmp_dir =
        g_build_filename(g_get_tmp_dir(), PACKAGE_NAME, (gchar *) 0);
    rmdir_recursive(ardesia_tmp_dir);
    g_free(ardesia_tmp_dir);
}
...
void annotate_configure_pen_options(
    AnnotateData * data)
{
...
{
    AnnotateSavepoint *savepoint =
        g_malloc((gsize) sizeof(AnnotateSavepoint));
...
void initialize_annotation_cairo_context(
    AnnotateData * data)
{
    if (data->annotation_cairo_context == NULL) {
        ...
        data->annotation_cairo_context =
            gdk_cairo_create(gtk_widget_get_window(data->annotation_window));
#endif
    ...
    )
{
    ...
    if (data->annotation_cairo_context) {
        ...
        if (savepoint->filename) {
            ...
            if (data->debug) {
                g_printerr
                    ("The save-point %s has been loaded from file\n",
                     savepoint->filename);
            }
...
void annotate_coord_list_prepend(
    AnnotateDeviceData * devdata,
    gdouble x,
    gdouble y,
    gdouble width,
...
void annotate_coord_dev_list_free(
    AnnotateDeviceData * devdata)
{
    ...
    gdouble pressure)
{
    ...
    if (devdata->coord_list != NULL) {
        AnnotatePoint *last_point =
            (AnnotatePoint *) g_slist_nth_data(devdata->coord_list, 0);
    ...
    gboolean stroke)
{
    if (!stroke) {
        ...
        AnnotatePoint *last_point =
            (AnnotatePoint *) g_slist_nth_data(devdata->coord_list, 0);
        if (last_point) {
            cairo_move_to(data->annotation_cairo_context,
    ...
    gdouble y)
{
    ...
    cairo_surface_t *source_surface =
        cairo_get_target(data->annotation_cairo_context);
    ...
    guint state)
{
    AnnotateDeviceData *masterdata =
        g_hash_table_lookup(data->devdatatable, masterdevice);
    AnnotateDeviceData *slavedata =
        g_hash_table_lookup(data->devdatatable, slavedevice);
...
void annotate_paint_context_free(
