    ...
    gdouble y)
{
    ...
    if ((x >= 0) && (x < info->width) && (y >= 0) && (y < info->height)) {
        ...
        while (sp > stack) {
            POP(y, x1, x2, dy);
            for (x = x1; (x >= 0)
                 && is_similar_to_old_pixel_value(info, x, y); x--) {
