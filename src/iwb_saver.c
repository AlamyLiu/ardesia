    ...
    gint index)
{
    ...
    gchar *file =
        g_strdup_printf("images/%s_%d_vellum.png", PACKAGE_NAME, index);
    ...
    gchar * background_image)
{
    ...
    image_destination_path =
        g_build_filename(img_dir_path, "ardesia_0_vellum.png", (gchar *) 0);
    ...
    if ((background_image) && (get_background_type() == 2)) {
        ...
        if (g_strcmp0(background_image, image_destination_path) != 0) {
            ...
            GFile *image_destination =
                g_file_new_for_path(image_destination_path);
...
static void add_savepoints(
    gint savepoint_number)
{
...
static void add_savepoint_reference(
    gint index)
{
    gchar *id = g_strdup_printf("id%d", index + 1);
...
static void add_savepoint_references(
    gint savepoint_number)
{
...
static void create_xml_content(
...
static void add_file_to_gst_outfile(
    GsfOutfile * out_file,
    gchar * path,
...
static void add_folder_to_gst_outfile(
    GsfOutfile * gst_outfile,
    gchar * working_dir,
    gchar * folder)
{
    GsfOutfile *gst_dir =
        GSF_OUTFILE(gsf_outfile_new_child(gst_outfile, folder, TRUE));
    ...
    gchar * iwb_location)
{
    ...
    gchar *first_savepoint_file =
        g_strdup_printf("%s%s%s_2_vellum.png", img_dir_path,
                        G_DIR_SEPARATOR_S,
                        PACKAGE_NAME);
    ...
    if ((file_exists(first_savepoint_file)) || (background_image)) {
        ...
        gchar *content_filepath =
            g_build_filename(project_tmp_dir, content_filename,
                             (gchar *) 0);
        ...
        if (iwb_location == NULL) {
            ...
            gchar *extension = "iwb";
            gchar *iwb_name = g_strdup_printf("%s.%s", get_project_name(),
                                              extension);

            /* The zip file is the iwb file located in the ardesia workspace. */
            iwb_file =
                g_build_filename(get_project_dir(), iwb_name, (gchar *) 0);
            g_free(iwb_name);
        } else {
