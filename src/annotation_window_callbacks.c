...
/* On configure event. */
G_MODULE_EXPORT gboolean on_configure(
    GtkWidget * widget,
    ...
    gpointer user_data)
{
    ...
    if (data->debug) {
        g_printerr
            ("DEBUG: Annotation window get configure event (%d,%d)\n",
...
/* On screen changed. */
G_MODULE_EXPORT void on_screen_changed(
    GtkWidget * widget,
...
G_MODULE_EXPORT gboolean on_expose(
    GtkWidget * widget,
    cairo_t * cr,
    gpointer user_data)
{
...
G_MODULE_EXPORT gboolean on_button_press(
    GtkWidget * win,
    GdkEventButton * ev,
    gpointer user_data)
{
    ...
    annotate_coord_list_prepend(masterdata,
                                ev->x,
                                ev->y, annotate_get_thickness(), pressure);

    return TRUE;
}

/* This shots when the pointer is moving. */
G_MODULE_EXPORT gboolean on_motion_notify(
    GtkWidget * win,
    GdkEventMotion * ev,
    gpointer user_data)
{
    ...
    if (ev->state != masterdata->state ||
        ev->state != slavedata->state || masterdata->lastslave != slave) {
    ...
    if (data->cur_context->type != ANNOTATE_ERASER) {
        ...
        if (masterdata->coord_list) {
            AnnotatePoint *last_point =
                (AnnotatePoint *) g_slist_nth_data(masterdata->coord_list,
                                                   0);
            gdouble tollerance = annotate_get_thickness();

            if (get_distance
                (last_point->x, last_point->y, ev->x, ev->y) < tollerance) {
    ...
    annotate_draw_line(masterdata, ev->x, ev->y, TRUE);
    annotate_coord_list_prepend(masterdata, ev->x, ev->y, selected_width,
                                pressure);
...
G_MODULE_EXPORT gboolean on_button_release(
    GtkWidget * win,
    GdkEventButton * ev,
    gpointer user_data)
{
    ...
    if (data->debug) {
        g_printerr
            ("Device '%s': Button %i Up at (x,y)= (%.2f : %.2f)\n",
             gdk_device_get_name(master), ev->button, ev->x, ev->y);
    }
#ifdef _WIN32
    if (inside_bar_window(ev->x_root, ev->y_root))
    ...
    if (lenght > 2) {
        ...
        gdouble distance =
            get_distance(ev->x, ev->y, first_point->x, first_point->y);
        ...
        if (distance > tollerance) {
            /* Different point. */
            annotate_draw_line(masterdata, ev->x, ev->y, TRUE);
            annotate_coord_list_prepend(masterdata, ev->x, ev->y,
                                        annotate_get_thickness(), pressure);
        } else {
            /* Rounded to be the same point. */
            closed_path = TRUE; // this seems to be a closed path
            annotate_draw_line(masterdata, first_point->x,
                               first_point->y, TRUE);
            annotate_coord_list_prepend(masterdata, first_point->x,
                                        first_point->y,
                                        annotate_get_thickness(), pressure);
        }
