    ...
    )
{
    if (background_data->background_cr) {
        ...
        gtk_window_set_opacity(GTK_WINDOW
                               (background_data->background_window), 1.0);

        gint new_height = 0;
        gint new_width = 0;
        new_height =
            gdk_window_get_height(gtk_widget_get_window
                                  (background_data->background_window));
        new_width =
        ...
        cairo_set_source_surface(background_data->background_cr,
                                 scaled_surface, 0.0, 0.0);
    ...
    )
{
    ...
    if (background_data->background_cr) {
        cairo_set_operator(background_data->background_cr,
                           CAIRO_OPERATOR_SOURCE);

#ifdef WIN32
        gdouble opacity = BACKGROUND_OPACITY;
        cairo_set_source_rgb(background_data->background_cr,
        ...
        gtk_window_set_opacity(GTK_WINDOW
                               (background_data->background_window), opacity);
#else
        gtk_window_set_opacity(GTK_WINDOW
                               (background_data->background_window), 1);
    ...
    )
{
    BackgroundData *background_data =
        g_malloc((gsize) sizeof(BackgroundData));
    gdk_rgba_parse( &(background_data->background_color), COLOUR_NULL);
    ...
    background_data->background_type = BG_NONE;
    ...
    )
{
    ...
    if (background_data->background_window_gtk_builder) {
        ...
        background_data->background_window_gtk_builder =
            (GtkBuilder *) NULL;
    }
    ...
    )
{
     ...
     *
     */
    gtk_window_set_opacity(GTK_WINDOW(background_data->background_window),
                           BACKGROUND_OPACITY);
    ...
    )
{
    ...
    background_obj =
        ...
        (background_data->background_window_gtk_builder, "backgroundWindow");
    background_data->background_window = GTK_WIDGET(background_obj);
    gtk_window_set_keep_above(GTK_WINDOW
                              (background_data->background_window), TRUE);

    /* This trys to set an alpha channel. */
    on_back_screen_changed(background_data->background_window, NULL,
                           background_data);

    gtk_window_set_opacity(GTK_WINDOW(background_data->background_window),
                           BACKGROUND_OPACITY);

    gtk_widget_set_size_request(background_data->background_window,
