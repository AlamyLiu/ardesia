 ...
 */
static gboolean is_a_rectangle(
    GSList * list,
    gdouble pixel_tollerance)
{
...
static gdouble calculate_medium_pression(
    GSList * list)
{
    guint i = 0;
...
static gboolean is_similar_to_a_regular_polygon(
    GSList * list,
    gdouble pixel_tollerance)
{
    guint i = 0;
...
static gdouble calculate_edge_degree(
    AnnotatePoint * point_a,
    ...
    GSList * list)
{
    ...
    first_point =
        allocate_point(inp_point->x, inp_point->y, inp_point->width,
                       inp_point->pressure);
 ...
 */
GSList *build_meaningful_point_list(
    GSList * list_inp,
    gboolean rectify,
    gdouble pixel_tollerance)
{
    ...
    AnnotatePoint *point_b =
        (AnnotatePoint *) g_slist_nth_data(list_inp, i + 1);
    ...
    if (lenght == 2) {
        AnnotatePoint *second_point =
            allocate_point(b_x, b_y, b_width, pressure);
        /* add a point with the coordinates of point_a. */
        ...
        AnnotatePoint *last_point =
            (AnnotatePoint *) g_slist_nth_data(list_inp, lenght - 1);
...
GSList *build_outbounded_rectangle(
    GSList * list)
{
    guint lenght = g_slist_length(list);
    AnnotatePoint *point =
        (AnnotatePoint *) g_slist_nth_data(list, lenght / 2);
    ...
    AnnotatePoint *point3 =
        allocate_point(minx, maxy, point->width, point->pressure);
    ret_list = g_slist_prepend(ret_list, point3);

    AnnotatePoint *point2 =
        allocate_point(maxx, maxy, point->width, point->pressure);
    ret_list = g_slist_prepend(ret_list, point2);

    AnnotatePoint *point1 =
        allocate_point(maxx, miny, point->width, point->pressure);
    ret_list = g_slist_prepend(ret_list, point1);

    AnnotatePoint *point0 =
        allocate_point(minx, miny, point->width, point->pressure);
...
gboolean is_similar_to_an_ellipse(
    GSList * list,
    gdouble pixel_tollerance)
{
    guint i = 0;
    ...
    gdouble pixel_tollerance)
{
    ...
    if (close_path) {
        ...
        for (i = 0; i < lenght; i++) {
            AnnotatePoint *point =
                (AnnotatePoint *) g_slist_nth_data(list_inp, i);
            AnnotatePoint *point_copy =
                allocate_point(point->x, point->y, point->width,
                               point->pressure);
    ...
    gdouble pixel_tollerance)
{
    GSList *meaningful_point_list =
        build_meaningful_point_list(list_inp, close_path,
                                    pixel_tollerance);

    if (meaningful_point_list) {

        if (rectify) {
            GSList *rectified_list =
                build_rectified_list(meaningful_point_list,
                                     close_path,
                                     pixel_tollerance);
