...
{
    return
        GTK_WIDGET(gtk_builder_get_object(bar_gtk_builder, BAR_WIDGET_NAME));
}
    ...
    )
{
    ...
    return gdk_pixbuf_get_from_window(root_window, 0, 0, width, height);
    ...
    GtkWidget * widget)
{
    ...
    const cairo_rectangle_int_t widget_rect =
        { x + 1, y + 1, width - 1, height - 1 };
    cairo_region_t *widget_reg = cairo_region_create_rectangle(&widget_rect);

    const cairo_rectangle_int_t ann_rect =
    ...
    )
{
    ...
    date = g_strdup_printf("%d-%d-%d_%d%s%d%s%d",
                           ...
                           t->tm_mon + 1,
                           t->tm_hour,
                           time_sep, t->tm_min, time_sep, t->tm_sec);
    ...
    GSList * attachment_list)
{
    ...
    gchar *args =
        g_strdup_printf("%s %s %s %s '%s'", mailer, subject_param, subject,
                        body_param, body);

    for (i = 0; i < attach_lenght; i++) {
        gchar *attachment = (gchar *) g_slist_nth_data(attachment_list, i);
        gchar *attachment_str =
            g_strdup_printf("%s '%s'", attach_param, attachment);
...
void send_artifacts_with_email(
    GSList * attachment_list)
{
    ...
    gchar * icon)
{
    ...
    if (!g_file_test(link_filename, G_FILE_TEST_EXISTS)) {
        ...
        xdg_create_desktop_entry(link_filename, "Application",
                                 PACKAGE_NAME, icon, exec);
        g_free(exec);
    }
